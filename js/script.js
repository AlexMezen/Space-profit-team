...
function formInfo() {
  const inputs = document.querySelectorAll(".input-roulette");
  const submitButton = document.querySelector(".submit-roulette");
  ...
  inputs.forEach(input => {
    input.addEventListener('input', (e) => {
      ...
      const {errorStatus, message} = validateForm(value, name);
      const regexNum = /^[0-9+]+$/;
      ...
      switch(name) {
        case 'name':
          ...
          inputStatus.name = errorStatus;
          addErrorMessage(message, parent, errorStatus)
          break;
        case 'phone':
      ...
      if(inputStatus.name && inputStatus.phone) {
...
function addErrorMessage(message, parent, status) {
  const div = document.createElement('div');
  if (!status && parent.children.length === 1) {

    div.classList.add('error-message');
    div.textContent = message;
    parent.append(div);
  } else if (status && parent.children.length === 2){
...
function validateForm(value, name) {
  switch(name) {
    case 'name':
      if (value.length < 2 && value.length !== 0) {
        return {
          errorStatus: false,
          message: "Введіть ім'я більше 2 символів"
        };
      } else {
        return {
          errorStatus: true,
...
function formSubmit() {
    ...
    const form = document.querySelector('.lid-form');
    const submitButton = document.querySelector(".submit-roulette");
    ...
    form.addEventListener('submit', (e) => {
      ...
      const transformBody = {
        ...
        'Стать': customer.gender,
        'Вік': customer.age,
        'Алергик': customer.allergy,
      }
      ...
      fetch('https://jsonplaceholder.typicode.com/posts', reqSeting)
      .then(res => res.json())
      .then(res => {
        notificationAftersendForm(form, 'done-message', 'Ваша заявка принята!')
    })
      .catch(() => {
        notificationAftersendForm(form, 'reject-message', 'Что-то пошло не так. Попробуйте ещё!')
      })
      .finally(() => {
        form.reset();
...
function notificationAftersendForm(parent, className, message) {
  const div = document.createElement('div');

  div.classList.add(className);
  div.textContent = message;
  parent.append(div);
  setTimeout(() => document.querySelector(`.${className}`).remove(), 4000)
}
...
const scrollButton = document.querySelector('.scroll-me.aba');
...
const coments = document.querySelectorAll('.comment-item');
const buttonShowMore = document.querySelector('.glow-button');
let offcet = 2

buttonShowMore.addEventListener('click', (e) => {
  offcet = offcet + 3;
  coments.forEach((coment, i) => {
    
    if (!coment.classList.contains('comment-item-show')) {

      if (offcet >= i) {
        coment.classList.add('comment-item-show')
      }
    }
  })

  if (offcet === coments.length - 1) {
